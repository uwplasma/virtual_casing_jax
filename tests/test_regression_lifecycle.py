...
def _case_vc():
    src_nt, src_np = 8, 6
    phi = 2.0 * jnp.pi * jnp.arange(src_nt) / src_nt
    theta = 2.0 * jnp.pi * jnp.arange(src_np) / src_np
    ...
    B0 = 0.05 * X + jnp.array([0.1, -0.03, 0.04])[:, None, None]
    vc = VirtualCasingJAX()
    vc.setup(3, 1, False, src_nt, src_np, X, src_nt, src_np, 4, 4)
    return vc, X, B0, (24, 24)
